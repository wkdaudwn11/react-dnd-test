...
// https://gist.github.com/brianpkelley/a5f57c5c3fc3b6e20cbdca294e49bbdd
...
class PinchZoomPan extends React.Component {
  ...
  handlePanMove(event) {
    ...
    this.setState({
      x: between(
        (this.props.width - this.state.width) / 2,
        (this.state.width - this.props.width) / 2,
        nextX
      ),
      y: between(
        (this.props.height - this.state.height) / 2,
  ...
  handlePinchMove(event) {
    ...
    /**
     * 2021-04-08 MJ
     * Pinch 액션? 이벤트?에 따라서 이미지 크기 조절하는 코드 시작
     */
    let newWidth = event.target.style.width;
    newWidth = newWidth.split("px");

    if (Array.isArray(newWidth) && newWidth.length === 2) {
      newWidth = Number(newWidth[0]);

      if (this.lastDistance < distance) {
        // 확대
        event.target.style.width = `${
          newWidth + (distance - this.lastDistance)
        }px`;
      } else {
        // 축소
        event.target.style.width = `${
          newWidth - (this.lastDistance - distance)
        }px`;
      }
    }
    /** 끝 */
  ...
  handleWheel(event) {
    ...
    const scale = between(
      MIN_SCALE - ADDITIONAL_LIMIT,
      MAX_SCALE + ADDITIONAL_LIMIT,
      this.state.scale + (event.deltaY > 0 ? 1 : -1) * SCALE_INCREMENT
    );
  ...
  zoom(scale, midpoint) {
    ...
    let nextState;
    if (
      this.props.width != this.initialState.width ||
      this.props.height != this.initialState.height
    ) {
      this.initialState = this.getInititalState();
    ...
    const tempX =
      nextState.x +
      inverse(midpoint.x * scale) * ((nextWidth - nextState.width) / nextWidth);
    const tempY =
      nextState.y +
      inverse(midpoint.y * scale) *
        ((nextHeight - nextState.height) / nextHeight);

    const nextX = between(
      (this.props.width - nextWidth) / 2,
      (nextWidth - this.props.width) / 2,
      tempX
    );
    const nextY = between(
      (this.props.height - nextHeight) / 2,
      (nextHeight - this.props.height) / 2,
      tempY
    );
    ...
    console.log(
      nextState.x +
        ...
        " *  " +
        (nextWidth - nextState.width) +
        " / " +
        nextWidth +
        ")",
      nextState.x +
        inverse(midpoint.x * scale) *
          ((nextWidth - nextState.width) / nextWidth)
    );
